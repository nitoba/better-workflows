...
export class WorkflowInterpreter {
  ...
  run<A>(
    ...
    return Effect.gen(function* () {
      const round = yield* durable(self.journal.get(self.executionId))
      return yield* Effect.withSpan(
        Effect.gen(function* () {
          yield* Effect.annotateLogs(
            Effect.logDebug('Workflow round started'),
            logAnnotations(TelemetryLogComponent.workflow, {
              [TelemetryAttributeKey.namespace]: round.namespace,
              [TelemetryAttributeKey.workflowName]: round.workflow_name,
              [TelemetryAttributeKey.workflowVersion]: round.version,
              [TelemetryAttributeKey.executionId]: round.execution_id,
              [TelemetryAttributeKey.executionChainId]: round.chain_id,
              [TelemetryAttributeKey.executionGeneration]: round.generation,
              [TelemetryAttributeKey.executionStatus]: round.state
            })
          )
          let ordinal = 0
                  ...
                  Effect.gen(function* () {
                    ...
                    yield* durable(
                      self.advanced
                        ...
                        .pipe(
                          Effect.withSpan(TelemetrySpanName.workflowChildStart, {
                            attributes: {
                              [TelemetryAttributeKey.parentExecutionId]: self.executionId,
                              [TelemetryAttributeKey.childExecutionId]: childId,
                              [TelemetryAttributeKey.workflowName]: contract.options.name,
                              [TelemetryAttributeKey.workflowVersion]: contract.options.version,
                              [TelemetryAttributeKey.stepId]: id,
                              [TelemetryAttributeKey.parentClosePolicy]: policy
                            },
                            kind: 'producer'
                          })
                        )
                    ...
                    const value = yield* DurableDeferred.await(childDeferred(id)).pipe(
                      Effect.withSpan(TelemetrySpanName.workflowChildWait, {
                        attributes: {
                          [TelemetryAttributeKey.parentExecutionId]: self.executionId,
                          [TelemetryAttributeKey.childExecutionId]: childId,
                          [TelemetryAttributeKey.workflowName]: contract.options.name,
            ...
            const context: WorkflowContext = {
              ...
              continueAsNew(input: unknown): Promise<never> {
                ...
                return dispatch(
                  Effect.gen(function* () {
                    ...
                    yield* durable(
                      self.journal
                        .continueAsNew(self.executionId, nextExecutionId, key, encoded)
                        .pipe(
                          Effect.withSpan(TelemetrySpanName.workflowContinueAsNew, {
                            attributes: {
                              [TelemetryAttributeKey.executionId]: self.executionId,
                              [TelemetryAttributeKey.nextExecutionId]: nextExecutionId,
                              [TelemetryAttributeKey.executionChainId]: current.chain_id,
                              [TelemetryAttributeKey.executionGeneration]: current.generation,
                              [TelemetryAttributeKey.nextGeneration]: current.generation + 1
                            }
                          })
                        )
                    )
                              ...
                              const result = yield* Effect.result(
                                Effect.gen(function* () {
                                  yield* Effect.withSpan(
                                      ...
                                      yield* self.transport
                                        ...
                                        .pipe(
                                          Effect.tapCause((cause) =>
                                            Effect.annotateLogs(
                                              Effect.logWarning('Activity dispatch failed'),
                                              logAnnotations(TelemetryLogComponent.workflow, {
                                    ...
                                    TelemetrySpanName.activityDispatch,
                                    {
                                      attributes: {
                                        [TelemetryAttributeKey.executionId]: self.executionId,
                                        [TelemetryAttributeKey.stepId]: id,
                                        ...
                                        [TelemetryAttributeKey.activityBusinessAttempt]: attempt
                                      },
                                      kind: 'producer',
              ...
              sleep(step, duration) {
                ...
                return command(step, 'timer', encode({ kind: 'timer', delay }), (id) =>
                  Effect.gen(function* () {
                    const protocol = yield* durable(
                      self.advanced.timer(self.executionId, id, delay)
                    )
                    if (protocol === 'legacy')
              ...
              child: (step, provider, input, options) =>
                child(step, provider, input, options, true),
          ...
          Effect.tapError((error) =>
            Effect.orDie(Effect.annotateCurrentSpan(TelemetryAttributeKey.failureCode, error.code))
          )
        ),
        TelemetrySpanName.workflowRound,
        {
          attributes: {
            [TelemetryAttributeKey.workflowName]: round.workflow_name,
            [TelemetryAttributeKey.workflowVersion]: round.version,
