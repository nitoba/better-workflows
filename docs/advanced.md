...
Examples use the real alpha.5 API. Workflows and activities are normal Nest providers. Keep I/O in activities and retain old handler versions while executions need replay. All persisted results and branch values must be plain JSON (or a root void result).
