...
Migrations are forward-only and transactional. The standalone admin API and CLI apply the journal and pinned native engine schemas without starting workers. Bootstrap can run them or validate first. The journal rejects newer/gapped ledgers and missing previously applied journal structures. Version 2 rebuilds the command ordinal index to include scope, preserves v1 command identities and records their timer protocol explicitly. Existing v1 timers continue through the native DurableClock protocol; newly recorded timers use a journal deadline and deferred outbox so test business time can advance them.
