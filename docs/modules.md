...
An activity worker loads the same queue identities and policies with `activities: [ReportActivitiesHandler]` and `execution.workflows.enabled: false` at the root. In contract-first mode, `ReportActivitiesHandler` is decorated with `@Activities(ReportActivities)` and implements the shared contract; the handler's methods do not repeat `@Activity` metadata. The orchestrator registers `activityContracts: [ReportActivities]`, while the worker registers only the implementation. Share a domain configuration factory/constant across roles to prevent default drift; a caller's defaults never replace the remote activity owner's policy. Imports/exports apply equally to contract-only features. Do not register the same contract in both `activities` and `activityContracts` inside one application.
...
Typical bootstrap errors include `FEATURE_NAME_REQUIRED`, `DUPLICATE_FEATURE`, `DUPLICATE_QUEUE_OWNER`, `QUEUE_NOT_VISIBLE`, `INVALID_FEATURE_EXPORT`, `UNKNOWN_QUEUE_OVERRIDE`, `ACTIVITY_KEY_REQUIRED`, `DUPLICATE_ACTIVITY_HANDLER`, `MISSING_ACTIVITY_HANDLER`, `ACTIVITY_HANDLER_REDECLARES_CONTRACT` and `UNSUPPORTED_SCOPE`. Cross-domain durable calls to unimported contracts report `ACTIVITY_NOT_VISIBLE` or `WORKFLOW_NOT_VISIBLE`; passing a handler where a contract is required reports `INVALID_ACTIVITIES_CONTRACT`. These include the relevant queue/provider/feature rather than falling back to arbitrary global handlers.
